    ...
    with app.app_context():
        ...
        for ad in pending_ads:
            if ad.payment_reference:
                ...
                logger.info(f"Payment info for ad ID {ad.id}: {payment_info}")
                if payment_info['response']['status'] == 'approved':
                    ad.is_paid = True
                    ad.status = 'publicado'
                    db.session.commit()
                    logger.info(f"Payment confirmed for ad ID: {ad.id}")
                else:
                    logger.info(f"Payment status for ad ID {ad.id} is: {payment_info['response']['status']}")
...
def home():
    if not current_user.is_authenticated:
        ...
        return redirect(url_for('index', condo_id=current_user.condo_id))
...
def create_ad(condo_id):
    ...
    if form.validate_on_submit():
        ...
        if highlight_plan and highlight_plan.price == 0:
            ...
            ad.expires_at = datetime.utcnow() + timedelta(weeks=1)
...
class UserForm(FlaskForm):
    ...
    email = StringField('Email', validators=[DataRequired(), Email()])
    password = PasswordField('Senha', validators=[Optional(), EqualTo('confirm', message='As senhas devem coincidir')])
    confirm = PasswordField('Repita a Senha')
...
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        user.username = form.first_name.data + ' ' + form.last_name.data
        user.email = form.email.data
...
@app.route('/conversation/<int:conversation_id>/send_message', methods=['POST'])
@login_required
def send_message(conversation_id):
    ...
    return jsonify({'status': 'error', 'message': 'Falha ao enviar a mensagem.'})
...
def get_messages(conversation_id):
    ...
    messages_data = [{'content': msg.content, 'sender': msg.sender.username, 'timestamp': msg.timestamp.isoformat()} for msg in messages]
